...
class ProjectPreview extends HTMLElement {
  constructor() {
    ...
    this.config = {
      width: '500px',
      color: '#008c86',
      font: '"Roboto Condensed", "Lato", "Helvetica Neue", Arial, Helvetica, sans-serif'
    };
  }

  // Appelé quand le composant est ajouté au DOM
  connectedCallback() {
    ...
    const projectSlug = this.getAttribute('project-slug'); // Récupérer le slug du projet
    this.render();
    if (!domain) {
      ...
      // Appel API pour récupérer les données du projet en ajoutant l'url prefixe 'geocontrib/' déjà inclue dans le chemin du thumbnail
  ...
  // Fonction pour rendre le composant initial
  render() {
  ...
  // Mise à jour du composant avec les données de l'API
