...
class ProjectAttributeAdminForm(forms.ModelForm):
    ...
    class Meta:
        ...
        # Specify a select form element since options cannot be set in model (user provided with javascript to insert in the select)
        widgets = {
            'default_filter_value': forms.Select(),
        }
